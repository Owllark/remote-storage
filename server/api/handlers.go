...
func Authenticate(w http.ResponseWriter, r *http.Request) {
	...
	hashedPassword, _ := main.database.GetHashedPassword(request.Name)
	if utils.Hash(request.Name+request.Password) != hashedPassword {
		w.WriteHeader(401)
		return
	}
	...
	tokenString, err := token.SignedString(main.jwtKey)
	if err != nil {
	...
	var client *main.User
	for i, _ := range main.users {
		if main.users[i].inf.Name == request.Name {
			client = &main.users[i]
		}
	}
...
func checkBearer(r *http.Request) (*types.User, error) {
	var res *types.User
	...
	for i, _ := range types.users {
		if types.users[i].types.Name == username {
			res = &types.users[i]
		}
	}
	return res, nil
